...
NDF.Reg <- function(pl,pr,n,Xtype,mu,sd=0.5,epsilon.x,epsilon.u,epsilon.v,seed){
  #####Data Generation And Accuracy computation for Simulation Regression Part Model IV #######
  #######Input#########
  ...
  pl <- pl
  pr <- pr
  #target dimensions to reduce, in Model 4, dl=dr=2
  dl <- 2
  dr <- 2
  ...
  f.true.3 <- log(1+u.train[1,])
  g.true.3 <- log(1+v.train[1,])
  f.true.4 <- (u.train[2,]+u.train[1,])
  g.true.4 <- (v.train[2,]+v.train[1,])
  
  f.true.t.1 <- (u.test[1,])^3
  g.true.t.1 <- (v.test[1,])^5
  f.true.t.2 <- exp(1+u.test[2,])
  g.true.t.2 <- exp(1+v.test[2,])
  f.true.t.3 <- log(1+u.test[1,])
  g.true.t.3 <- log(1+v.test[1,])
  f.true.t.4 <- (u.test[2,]+u.test[1,])
  g.true.t.4 <- (v.test[2,]+v.test[1,])
  
  Y.n <- array(data = 0,dim = ncol(X.tr))
  for (i in 1:n) {
    Y.n[i] <- log(1+sum((lambda.train*(f.true.1*g.true.1+f.true.2*g.true.2+f.true.3*g.true.3+f.true.4*g.true.4))[(r*(i-1)+1):(r*i)]))
  }
  Y <- (Y.n)+rnorm(n,0,sd)*sd(Y.n)
  Y.t <- array(data = 0,dim = ncol(X.te))
  for (i in 1:n.t) {
    Y.t[i] <- log(1+sum((lambda.test*(f.true.t.1*g.true.t.1+f.true.t.2*g.true.t.2+f.true.t.3*g.true.t.3+f.true.t.4*g.true.t.4))[(r*(i-1)+1):(r*i)]))
  }
  ...
  gf.true <- cbind(as.matrix(f.true.t.1*g.true.t.1),as.matrix(f.true.t.2*g.true.t.2),as.matrix(f.true.t.3*g.true.t.3),as.matrix(f.true.t.3*g.true.t.3))
  ...
  acc2 <- (cor(as.matrix(Y.test.pred),as.matrix(Y.test)))
...
# Stop Cluster
stopCluster(cl)
write.csv(results, file = "Reg4.csv")
